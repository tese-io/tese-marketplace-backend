...
export default async function returnRequestUpdatedHandler({
  event,
  container,
  ...
  try {
    ...
    if (request.order?.email) {
      notifications.push({
        ...
        content: { subject: `Return request ${resolvedStatus} — order #${orderLabel}` },
        data: {
          data: {
            title: `Your return request is now ${resolvedStatus}`,
            body: `Hi ${request.order.customer?.first_name || "there"},\n\nYour return request for order #${orderLabel} was updated to "${resolvedStatus}".`,
    ...
    if (request.seller?.email) {
      notifications.push({
        ...
        data: {
          data: {
            title: `Return request ${resolvedStatus}`,
            body: `Hi ${request.seller.name || "there"},\n\nThe return request for order #${orderLabel} is now "${resolvedStatus}".`,
            cta_label: "View order",
            cta_url: sellerUrl,
            store_name: storeData.store_name,
            storefront_url: storeData.storefront_url,
    ...
    console.error("return request updated email failed", error)
  }
}

export const config: SubscriberConfig = {
  event: OrderReturnRequestEvents.UPDATED,
  context: { subscriberId: "return-request-updated-handler-resend" },
}
